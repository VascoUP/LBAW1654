...
function addTaskWithoutDescription($name, $priority, $effort, $taskStatus, $itID){
	try {
		global $conn;
		
		$stmt = $conn->prepare("INSERT INTO Task(iterationID, priority, name, effort, taskStatus)
								VALUES (:iterationID, :priority, :name, :effort, :taskStatus)");						
		$stmt->bindParam(':iterationID', $itID);
		$stmt->bindParam(':priority', $priority);
		$stmt->bindParam(':name', $name);
		$stmt->bindParam(':effort', $effort);
